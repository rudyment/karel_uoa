...
#U:/p5_Python/karel_uoa3/__init__.py
"""
...
from typing import Protocol, runtime_checkable, overload, Sequence
...
###########################################################################q
# ROZHRANÍ SVĚTA ROBOTŮ
###########################################################################q

@runtime_checkable
class IRobotWorld(Protocol):
    """Základní svět robotů umožňující nezávislý pohyb několika robotů
    implementujících protokol IKarel.
    """

    @abstractmethod
    def __repr__(self) -> str:
        """V podpisu světa robotů bude počet jeho řádků a sloupců
        a počet v něm se vyskytujících robotů.
        """

    @abstractmethod
    def add_robot(self, row:int, col:int, dir4:Dir4, color:Color,
                        RobotClass:type) -> None:
        """Přidá do aktuálního světa robotů instanci třídy robotů
        zadané v argumentu RobotClass, umístěnou na souřadnice [row, col],
        otočenou do směru dir4 a vybarveného barvou `color`.
        Záporné souřadnice zadávají pozici vůči pravému, resp. spodnímu okrají
        světa robotů.
        Pokud se na zadaných souřadnicích vyskytuje jiný robot nebo zeď,
        vyvolá výjimku.
        Očekává se, že třída RobotClass má bezparametrický konstruktor,
        a implementuje protokol IKarel, případně IKarelCM.
        """
...
###########################################################################q
# SPRÁVCE SVĚTŮ, KTERÝ UMÍ VYTVÁŘET A RUŠIT SVĚTY ROBOTA KARLA
###########################################################################q

@runtime_checkable
class IRobotWorldManager(Protocol):
    """Správce světů robotů, který zabezpečí, že v každém okamžiku existuje
    nejvýše jeden aktivní svět robotů.
    """

    @abstractmethod
    def active_world(self) -> IRobotWorld | None:
        """Vrací odkaz na aktuální aktivní svět; není-li takový, vrátí None.
        """

    @overload
    def new_world(self, rows:int=DEFAULT_ROWS, cols:int=DEFAULT_COLS) -> None:
    ...
    @overload
    def new_world(self, row0:str, *rows:str) -> None:
        """Případnou existující aktivní instanci dvorku nejprve deaktivuje.
    ...
    @overload
    def new_world(self, row0:Sequence[int], *rows:Sequence[int]) -> None:
        """Případnou existující aktivní instanci dvorku nejprve deaktivuje.
        Vytvoří svět (dvorek) zaplněný značkami,
        v němž nezáporné prvky jednotlivých seznamů indikují počet značek
        na daném poli a záporné prvky indikují zeď nebou jinou překážku.
        """

    @abstractmethod
    def new_world(self, *fields: int | str | Sequence[int]) -> None:
        """Případnou existující aktivní instanci dvorku nejprve deaktivuje.
        Poté nový šachovnicový svět podle požadavků popsaných
        ve výše uvedených přetížených definicích.
        """

    @abstractmethod
    def set_world_class(self, WorldClass:type) -> None:
        """Nastaví třídu, jejíž instance bude vytvářet new_world()."""
